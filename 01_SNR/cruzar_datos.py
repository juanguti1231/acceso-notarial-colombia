...
# 1. POBLACIÓN
# ─────────────────────────────────────────
pob_raw = pd.read_excel('01_SNR/dane/poblacion_departamentos.xlsx',
                        sheet_name='PobDepartamentalxÁrea', header=7)
pob_raw.columns = ['DP','Departamento','Año','Area','Poblacion']
pob = pob_raw[pob_raw['Area']=='Total'][['Departamento','Año','Poblacion']].copy()
...
# 2. PIB
# ─────────────────────────────────────────
pib_raw = pd.read_excel('01_SNR/dane/pib_departamentos.xlsx',
...
pib_raw = pib_raw[pib_raw['Departamento'] != 'COLOMBIA'].dropna(subset=['Departamento'])
...
# 3. IPM
# ─────────────────────────────────────────
ipm_raw = pd.read_excel('01_SNR/dane/ipm_departamentos.xlsx',
...
ipm_cols = ['Departamento'] + [f'IPM_{a}_Total' for a in años_ipm
...
# 5. SNR 2025
# ─────────────────────────────────────────
snr = pd.read_csv('01_SNR/Actos_Jurídicos_Notariales_20260609.csv',
...
df['compraventas_per_capita'] = (df['Compraventa'] / df['Poblacion']).round(6)
df['PIB_per_capita'] = (df['PIB_miles_millones'] * 1e9 / df['Poblacion']).round(0)
...
df = df.sort_values('actos_per_capita', ascending=False).reset_index(drop=True)
...
df.to_csv('06_Consolidado/indice_acceso_notarial_2025.csv',
          index=False, encoding='utf-8-sig')

conn = sqlite3.connect('06_Consolidado/notaria74.db')
df.to_sql('indice_acceso', conn, if_exists='replace', index=False)
pob.to_sql('poblacion_departamentos', conn, if_exists='replace', index=False)
pib.to_sql('pib_departamentos', conn, if_exists='replace', index=False)
ipm.to_sql('ipm_departamentos', conn, if_exists='replace', index=False)
conn.close()

print("\n✓ 06_Consolidado/indice_acceso_notarial_2025.csv")
print("✓ 06_Consolidado/notaria74.db actualizada con 4 tablas")